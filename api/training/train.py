"""
BiLSTM v7 — best model at 69.2% val on asl100
"""
...
OUTPUT_MODEL  = os.getenv("OUTPUT_MODEL",  "lstm_model_v7.pth")
...
def time_warp(frames, sigma=0.2):
    T     = frames.shape[0]
...
def cutout(frames, num_cuts=2, cut_len=5):
    frames = frames.clone()
    T = frames.shape[0]
...
def augment_frames(frames):
    ...
    if random.random() < 0.5:  frames = time_warp(frames, sigma=0.15)
    if random.random() < 0.4:  frames = joint_dropout(frames, p=0.1)
    if random.random() < 0.4:  frames = cutout(frames, num_cuts=2, cut_len=4)
